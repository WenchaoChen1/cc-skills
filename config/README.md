...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `project_root` | `cc-cache-doc` | 项目配置根目录，可为空（空 = 项目根目录） |
| `personal_root` | `.cc-cache-doc` | 个人配置根目录，可为空（空 = 用户 home 目录） |
| `features` | `features` | 功能目录名，必填 |
| `rules` | `rules` | 规则目录名，必填 |
| `standards` | `standards` | 规范目录名，必填 |
...
- `project_root` 和 `personal_root` 可以为空字符串：
  - `"project_root": ""` → 功能目录直接在项目根目录下：`features/{name}/requirement/`
  - `"personal_root": ""` → 个人规则直接在用户 home 下：`~/rules/`
- `features`、`rules`、`standards` **不能为空**，必须有值
